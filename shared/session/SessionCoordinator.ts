...
export type { AsrProviderId, DisplaySettings, FollowState, TranscriptSource } from '../domain/types.js';
...
export class SessionCoordinator {
  ...
  adoptAuthoritativeState(state: SessionCoordinatorState) {
    this.currentTokenIndex = this.clampToken(state.currentTokenIndex);
    this.followState = state.followState;
    this.alignment = state.alignment;
    this.transcriptBuffer = [...state.transcriptBuffer];
    this.latestTranscript = state.latestTranscript;
    this.alignmentBufferDebug = { ...state.alignmentBufferDebug };
    this.manualAnchor = state.manualReacquireAnchor ? { ...state.manualReacquireAnchor } : null;
    this.movementDecision = state.movementDecision ? { ...state.movementDecision } : null;
    this.movementHistory = state.movementDecisionHistory.map((decision) => ({ ...decision }));
    this.sessionRevision = Math.max(this.sessionRevision, state.sessionRevision);
    this.manuscriptRevision = Math.max(this.manuscriptRevision, state.manuscriptRevision);
    return this.getState();
  }
