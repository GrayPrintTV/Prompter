...
- Local Whisper: mic chunks from renderer to Electron main, then Python faster-whisper sidecar; local and optional.
...
- Local Whisper bridge should be manually smoke-tested after full Electron restart.
- Chromium MediaRecorder currently emits WebM/Opus by default; faster-whisper/ffmpeg compatibility for those chunks still needs real-world testing.
- Local Whisper sidecar needs a self-test path independent of microphone capture.
...
- Run a Local Whisper sidecar self-test with a known audio fixture.
- Verify MediaRecorder chunk format can be decoded by faster-whisper in the user's Python environment.
...
8. Confirm Sidecar moves from Starting to Model loading to Ready, or displays a clear Python/faster-whisper setup error.
9. Speak one or two manuscript sentences.
10. Confirm chunk counters advance: Recorded, Sent, Sidecar, Returned.
11. Confirm `Heard` shows raw transcript text or `[empty transcript]`.
12. Confirm Manual and Mock still work if Local Whisper fails.
